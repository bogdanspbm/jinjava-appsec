...
public class MapFilter implements Filter {
   ...
   public Object filter(Object var, JinjavaInterpreter interpreter, String... args) {
      ...
      if (args.length == 0) {
         throw new InterpretException(this.getName() + " filter requires name of filter or attribute to apply to given sequence");
      } else {
         String attr = args[0];
         ...
         ArrayList result;
         Object val;
         for(result = new ArrayList(); loop.hasNext(); result.add(val)) {
            val = loop.next();
            if (apply != null) {
