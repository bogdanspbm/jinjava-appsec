...
public class Functions {
   ...
   public static String renderSuperBlock() {
      ...
      if (superBlock != null) {
         Iterator var3 = superBlock.iterator();

         while(var3.hasNext()) {
            Node n = (Node)var3.next();
            result.append((Object)n.render(interpreter));
         }
      }
   ...
   public static String dateTimeFormat(Object var, String... format) {
      ...
      if (d == null) {
         ...
         Locale locale = (Locale)JinjavaInterpreter.getCurrentMaybe().map(JinjavaInterpreter::getConfig).map(JinjavaConfig::getLocale).orElse(Locale.ENGLISH);
         return format.length > 0 ? StrftimeFormatter.format(d, format[0], locale) : StrftimeFormatter.format(d, locale);
      }
   }
   ...
   public static long unixtimestamp(Object var) {
      ZonedDateTime d = getDateTimeArg(var);
      return d == null ? 0L : d.toEpochSecond() * 1000L;
   }

   @JinjavaDoc(
      value = "truncates a given string to a specified length",
      params = {@JinjavaParam("s"), @JinjavaParam(
   value = "length",
   type = "number",
   defaultValue = "255"
), @JinjavaParam(
      ...
      if (var instanceof String) {
         ...
         if (arg.length > 0) {
            try {
               ...
               Logging.ENGINE_LOG.warn("truncate(): error setting length for {}, using default {}", arg[0], 255);
            }
         }
         ...
         if (arg.length > 1) {
            try {
               ...
               Logging.ENGINE_LOG.warn("truncate(); error setting killwords for {}", arg[1]);
            }
         }
   ...
   public static List<Integer> range(Object arg1, Object... args) {
      ...
      if (step == 0) {
         ...
         if (start < end) {
            ...
            for(i = start; i < end && result.size() < 1000; i += step) {
               result.add(i);
            }
         } else {
            if (step > 0) {
